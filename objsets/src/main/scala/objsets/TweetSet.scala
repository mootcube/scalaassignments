...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet = if(isEmpty)that else if(that.isEmpty)this else union (that.tail) incl (that.head)
  ...
  def ascendingByRetweet0(acc:Trending):Trending={
    if(isEmpty)acc else remove(findMin).ascendingByRetweet0(acc+findMin)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = if(p(head)) tail.filter0(p,accu.incl(head)) else tail.filter0(p,accu)
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(t=>google.exists(t.text.contains))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(t=>apple.exists(t.text.contains))
